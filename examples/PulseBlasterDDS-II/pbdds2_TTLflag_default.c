...
main (int argc, char *argv[])
{
  ...
  printf ("Copyright (c) 2010 SpinCore Technologies, Inc.\n\n");
	...
	printf("This program will produce a 1.0 MHz signal on both channels that is on for \n"
	       "10 microseconds, off for 10 microseconds, and then repeats this pattern. \n"
		   "Channel 1 and Channel 2 will alternate being on and off. This example shows\n"
		   "the ability to change the 'return-to' states of the TTL flag bits with only\n"
		   "one simple write to the board. Currently they are set to return to 0xA5A \n"
		   "which corresponds to the bit pattern 1010 0101 1010 (leftmost bit is most \n"
		   "significant bit, i.e. FLAG 11). This means that when the PulseBlaster Core \n"
		   "is in a STOPPED state, the flag bits will return to the specified output \n"
		   "pattern.\n\n");
    printf("NOTE: Once the return to TTL flag state register has been changed, it will \n"
	       "      retain that changed value until it is changed again, or the board is \n"
		   "      powered off. It is important to terminate all signals properly (i.e. \n"
		   "      with a 50 Ohm terminating resistor at the end of your cable). A \n"
		   "      reconstructing filter must also be used at the RF outputs. When \n"
		   "      viewing the RF signals on an oscilloscope, the oscilloscope should be\n"
		   "      triggered using a TTL signal\n\n");
