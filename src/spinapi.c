...
pb_start_programming (int device)
{
  ...
  if (cur_device != -1)
    {
		debug(DEBUG_WARNING, "pb_start_programming: pb_start_programming() called without previous stop");
    }
	  ...
	  if (device == PULSE_PROGRAM)
        {
          ...
          printf("sofsafe: pb_start_programming() 3 //RESET: %d\n", return_value);
          if (return_value)
            {
          ...
          printf("sofsafe: pb_start_programming() 4 //RESET COUNTER: %d\n", return_value);
          if (return_value)
            {
...
pb_inst_pbonly64 (__int64 flags, int inst, int inst_data, double length)
{
  ...
  // pb_inst_direct may index from 0-2 into the pflags parameter
  pad_flags (flags, flags_padded);
  return pb_inst_direct (flags_padded, inst, inst_data, delay);
}
